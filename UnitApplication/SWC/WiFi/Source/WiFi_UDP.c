 ...
 * (`PICO_W_AP_TCP_SERVER_ADDRESS`) to an IP address or by setting it directly to a default value 
...
bool udp_client_send(const char* message) 
{
    ...
    if (!addr_initialized) {
        ...
        if (ipaddr_aton(PICO_W_AP_TCP_SERVER_ADDRESS, &dest_addr) != 1) {
